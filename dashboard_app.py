...
with st.container(border=False):
    ...
    try:
        hist = build_mark_to_market_equity_curve(
            trades=raw_trades,
            current_equity=equity,
            days_back=int(chart_settings["days_back"]),
            timeframe=str(chart_settings["timeframe"]),
        )
    except Exception:
        hist = pd.DataFrame(columns=["time", "equity", "source"])
    ...
    if hist.empty:
        st.info(
            "No usable portfolio graph yet. Once there are trades or Alpaca returns history, the graph will move."
        )
    ...
    try:
        if not hist.empty and "source" in hist.columns:
            source_label = str(hist["source"].iloc[-1])
    except Exception:
        source_label = ""
